...
set :repository,          "git@github.com:markmcspadden/coding-mentors.git"
set :branch,			        "master"

# More symlinks
desc "Symlink recaptcha key"
task :symlink_recaptcha_key, :roles => :app, :except => {:no_symlink => true} do
  run <<-CMD
    ln -nfs #{shared_path}/config/initializers/recaptcha_production.rb #{release_path}/config/initializers/recaptcha_production.rb;
  CMD
end
desc "Symlink site keys"
task :symlink_site_keys, :roles => :app, :except => {:no_symlink => true} do
  run <<-CMD
    ln -nfs #{shared_path}/config/initializers/site_keys_production.rb #{release_path}/config/initializers/site_keys_production.rb;
  CMD
end

after "deploy:symlink", "symlink_recaptcha_key"
after "deploy:symlink", "symlink_site_keys"
